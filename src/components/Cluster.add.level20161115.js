...
class AddLevel extends Component {
    ...
    editThisLevel(val,name,index){
       ...
       /* var data={
        ...
        var _id=val.substring(0,val.lastIndexOf("&devG&")),_name=val.substring(val.lastIndexOf("&devG&")+6);
        dispatch(editLevel({ [name+"_id"]:_id,[name+"_name"]:_name},index));
        //校验
        //if(name=="topology"&&){}
       // console.log(_id);
                   ...
                   // console.log(data);
                    if(data.info.status=="success"){
                        ...
                        for(var i=0;i<data.length;i++){
                            ...
                            _topoOpc.push(<Option key={data[i]._id+"&devG&"+data[i].name}>{data[i].name}</Option>);
    ...
    render() {
        //console.log(this.props);
        const {validator,thisForm,levels,cluster,dispatch,index,devs_group_id,devs_group_name,level,topology_id,topology_name,topoArr}=this.props;
        return (
                    <tr>
                            ...
                            <div className="">
                                ...
                                <ValidateItem validator={validator} thisForm={thisForm} field="devs_group" index={index} itemType="1">
                                    <Select
                                        ...
                                        >
                                        {level<=2?bdevGroupOption:zdevGroupOption}
                                    </Select>
                                </ValidateItem>
                            ...
                            <div className="">
                                ...
                                <ValidateItem validator={validator} thisForm={thisForm} field="topology" index={index} itemType="1">
                                    <Select
                                        ...
                                        notFoundContent="无搜索结果！"
                                        value={!topology_id?"":topology_id+"&devG&"+topology_name}
                                        disabled={level<=2?true:false}
                                        >
                                        {
                                            /*topoOpction*/
                                            level<=2?<Option value="" key="" >禁用</Option>:topoArr
                        ...
                        <td className="text-center">
                            <Tooltip placement="top" title="添加上层">
                                <i className="upLevel" onClick={()=>{this.addLevel(index);}}></i>
                            </Tooltip>
                           {level!=2?
